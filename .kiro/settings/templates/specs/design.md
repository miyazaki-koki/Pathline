...
## Testing Strategy (必須セクション)

> このセクションは必須です。全機能に対して網羅性の高いテスト仕様を記載してください。
...
### 1. Unit Tests（単体テスト）- テストコード作成必須

各コンポーネント・関数・メソッドに対する単体テストを記載。

| テストID | テスト対象 | テスト観点 | テストケース | 入力値 | 期待結果 | 優先度 |
|----------|-----------|-----------|-------------|-------|---------|-------|
| UT-001 | [関数/メソッド名] | 正常系 | [ケース説明] | [入力] | [期待結果] | High/Med/Low |
| UT-002 | [関数/メソッド名] | 異常系 | [ケース説明] | [入力] | [期待結果] | High/Med/Low |
| UT-003 | [関数/メソッド名] | 境界値 | [ケース説明] | [入力] | [期待結果] | High/Med/Low |
...
### 2. Integration Tests（統合テスト）- テストコード作成必須

API連携、Repository層、Service層の結合テストを記載。

| テストID | テスト対象 | 連携コンポーネント | テストシナリオ | 前提条件 | 期待結果 | 優先度 |
|----------|-----------|------------------|--------------|---------|---------|-------|
| IT-001 | [API/Service名] | [連携先] | [シナリオ説明] | [前提条件] | [期待結果] | High/Med/Low |
| IT-002 | [API/Service名] | [連携先] | [シナリオ説明] | [前提条件] | [期待結果] | High/Med/Low |
...
### 3. E2E Tests（End-to-Endテスト）- テスト仕様書必須

ユーザーフロー全体のシナリオテストを記載。テストコード作成は任意。

| テストID | シナリオ名 | ユーザーストーリー | 前提条件 | テスト手順 | 期待結果 | 優先度 |
|----------|-----------|------------------|---------|-----------|---------|-------|
| E2E-001 | [シナリオ名] | [ユーザーとして〜したい] | [前提条件] | 1. [手順1]<br>2. [手順2]<br>3. [手順3] | [期待結果] | High/Med/Low |
...
### 4. Manual Tests（打鍵テスト）- テスト仕様書必須

UI/UXの目視確認、ブラウザ互換性テストを記載。

| テストID | 確認項目 | 確認内容 | 確認環境 | 期待結果 | 優先度 |
|----------|---------|---------|---------|---------|-------|
| MT-001 | [UI要素名] | [確認内容] | Chrome/Safari/Mobile | [期待結果] | High/Med/Low |
| MT-002 | [UI要素名] | [確認内容] | Chrome/Safari/Mobile | [期待結果] | High/Med/Low |
...
### 5. Performance Tests（性能テスト）- 該当する場合のみ

| テストID | テスト対象 | 測定項目 | 目標値 | テスト条件 |
|----------|-----------|---------|-------|-----------|
| PT-001 | [API/画面名] | レスポンスタイム | < 200ms | 同時接続数: 100 |

### テストカバレッジ目標

| テストレベル | カバレッジ目標 | 備考 |
|-------------|--------------|------|
| Unit Tests | 80%以上 | クリティカルパス必須 |
| Integration Tests | 主要フロー100% | API全エンドポイント |
| E2E Tests | 主要ユーザーフロー | 仕様書ベース |
| Manual Tests | 全UI要素 | リリース前必須 |
